    ...
    <div>
      ...
      <nav class="flex flex-col space-y-2 px-4">
        <router-link
          ...
          :key="link.name"
          :to="link.path"
          class="group relative px-4 py-3 rounded-lg transition-all duration-300 no-underline text-inherit overflow-hidden"
          :class="{
            'bg-tertiary font-semibold shadow-lg shadow-tertiary/30': isActive(
              link.path,
            ),
            'hover:bg-secondary hover:translate-x-1': !isActive(link.path),
          }"
        >
          <div
            class="absolute inset-0 bg-gradient-to-r from-tertiary/0 via-tertiary/10 to-tertiary/0 opacity-0 group-hover:opacity-100 transition-opacity duration-300"
            v-if="!isActive(link.path)"
          ></div>
          <span class="relative z-10 flex items-center gap-2">
            <span
              class="w-1.5 h-1.5 rounded-full bg-tertiary transition-all duration-300"
              :class="
                isActive(link.path)
...
nav::-webkit-scrollbar-thumb:hover {
  background: var(--color-tertiary-dark, #7c3aed);
}
</style>
